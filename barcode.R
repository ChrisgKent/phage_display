...
file <- "VIRSCAN1_S1_L001_R1_001.fastq"
...
barcode_count <- barcode_count[order(barcode_count$count),]

write_delim(as.data.frame(barcode_count), "files/barcode_count.txt", col_names = FALSE)

# This picks the 20 barcodes with the highest count, and assigns them to a list
# As strings for memory reasons.
top_20 <- tail(barcode_count,20)
